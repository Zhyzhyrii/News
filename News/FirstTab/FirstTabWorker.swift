...
class FirstTabWorker: Parser {
    ...
    func getNewsFromDBOrNetworkFor(indexOfTab: Int, completionHandler: ([New]?, GetNewsError?) -> Void) -> Void  {
        guard let feedModel = getSelectedFeedModel(indexOfTab: indexOfTab) else { // if the source is not selected for the specific tab
            completionHandler(nil, GetNewsError.noSourceIsSelected)
            return
        }
