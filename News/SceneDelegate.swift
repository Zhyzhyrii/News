...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        let viewController = self.window!.rootViewController as! UITabBarController
            ...
            for index in 0...tabBarItems.count - 2 {
                if let savedFeeds = StorageManager.shared.getSavedFeeds(forKey: index) {
                    if let selectedFeed = savedFeeds.first(where: { (feed) -> Bool in
                        ...
                        viewController.tabBar.items?[index].title = selectedFeed.feedName
                    }
                } else {
                    viewController.tabBar.items?[index].title = "Item"
                }
            }
        }
